...
def create_table_if_not_exist(db_name :str, db_table : str):
    con = mysql.connect()
    cursor = con.cursor()
    cursor.execute(f'CREATE TABLE IF NOT EXISTS {db_table} (ID INTEGER PRIMARY KEY AUTO_INCREMENT,'
...
def value_of_columns(db_name : str, db_table : str, name_cols : list[str]):
    if name_cols == []:
        return '*'
    try:
        query = f"SHOW columns FROM {db_table}"
        con, cursor = connect_database(db_name)
        cursor.execute(query)  
        data = cursor.fetchall()
        lst_col = [col[0].lower() for col in data]
        for col in name_cols:
            if col.lower() not in lst_col:
                return Error(f"{col} is invalid")
...
def update(db_name : str, db_table : str, values : list[str], id: int):
    ...
    if response:
        ...
        return Error(f"Id {id} doesn't exist in the table {db_table}")
